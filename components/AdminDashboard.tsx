...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
    ...
    const [expSubjId, setExpSubjId] = useState('');
    ...
    // Safely set initial selections when data loads
    useEffect(() => {
        if (departments.length > 0) {
            if (!expDeptId) setExpDeptId(departments[0].id);
            if (!subjDeptId) setSubjDeptId(departments[0].id);
        }
    }, [departments, expDeptId, subjDeptId]);
    ...
    }, [expDeptId, departments, availableSubjects, expSubjId]);
        ...
        <div className="max-w-4xl mx-auto">
            <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg dark:bg-gray-900">
                ...
                <div className="border-b border-gray-200 dark:border-gray-700">
                    <nav className="-mb-px flex space-x-6" aria-label="Admin Tabs">
                        {TABS.map((tab) => (
                            <button
                                key={tab.id}
                ...
                <div className="mt-8">
                    ...
                    {activeTab === 'experiments' && (
                         <section aria-labelledby="create-experiment-heading">
                            <form onSubmit={handleCreateExperimentSubmit} className="space-y-6">
                                <h2 id="create-experiment-heading" className="text-2xl font-bold text-gray-800 dark:text-gray-200">Create New Experiment</h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <select id="exp_department" value={expDeptId} onChange={(e) => setExpDeptId(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200" aria-label="Select department for new experiment">
                                    ...
                                    <div>
                                        ...
                                        <select id="exp_subject" value={expSubjId} onChange={(e) => setExpSubjId(e.target.value)} disabled={availableSubjects.length === 0} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md disabled:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:disabled:bg-gray-800" aria-label="Select subject for new experiment">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="title" value={expTitle} onChange={(e) => setExpTitle(e.target.value)} placeholder="e.g., Implement a Singly Linked List" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-400" aria-label="Experiment title"/>
                                </div>
                                <div>
                                    <label htmlFor="objective" className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Objective</label>
                                    <textarea id="objective" rows={3} value={expObjective} onChange={(e) => setExpObjective(e.target.value)} placeholder="Describe the main goal of this experiment..." className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-400" aria-label="Experiment objective"/>
                                </div>
                                <div className="flex justify-end pt-2"><button type="submit" className="px-6 py-2 bg-primary-600 text-white font-semibold rounded-lg shadow-md hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors" aria-label="Create experiment">Create Experiment</button></div>
                            </form>
                            <div className="mt-8 pt-6 border-t dark:border-gray-700"><h3 className="text-xl font-bold text-gray-800 dark:text-gray-200">Existing Experiments</h3>
                                <div className="mt-4 space-y-2 max-h-96 overflow-y-auto pr-2">{departments.map(dept => dept.subjects.map(subj => subj.experiments.map(exp => (
                                    <div key={exp.id} className="flex justify-between items-center p-3 bg-gray-50/50 border rounded-lg dark:bg-gray-800/50 dark:border-gray-700">
                                        ...
                                        <button onClick={() => confirmDelete(`Are you sure you want to delete the experiment "${exp.title}"?`, () => onDeleteExperiment(subj.id, exp.id))} className="flex items-center space-x-1 text-red-600 hover:text-red-800 font-medium text-sm p-1 -m-1 rounded-md transition-colors" aria-label={`Delete experiment ${exp.title}`}><TrashIcon className="w-4 h-4" /></button>
                                    </div>
                                ))))}</div>
                            </div>
                        </section>
                    ...
                    {/* MANAGE SUBJECTS TAB */}
                    {activeTab === 'subjects' && (
                         <section aria-labelledby="create-subject-heading">
                            <form onSubmit={handleCreateSubjectSubmit} className="space-y-6">
                                <h2 id="create-subject-heading" className="text-2xl font-bold text-gray-800 dark:text-gray-200">Create New Subject (Lab)</h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label htmlFor="subj_department" className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Department</label>
                                        <select id="subj_department" value={subjDeptId} onChange={(e) => setSubjDeptId(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200" aria-label="Select department for new subject">
                                            {departments.map(dept => <option key={dept.id} value={dept.id}>{dept.name}</option>)}
                                        </select>
                                    </div>
                                    <div>
                                        <label htmlFor="subj_name" className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Subject Name</label>
                                        <input type="text" id="subj_name" value={subjName} onChange={(e) => setSubjName(e.target.value)} placeholder="e.g., Data Structures Lab" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-400" aria-label="Subject name"/>
                                    </div>
                                    <div>
                                        <label htmlFor="subj_code" className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Subject Code</label>
                                        <input type="text" id="subj_code" value={subjCode} onChange={(e) => setSubjCode(e.target.value)} placeholder="e.g., CS201L" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-400" aria-label="Subject code"/>
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input type="url" id="subj_drive" value={subjDrive} onChange={(e) => setSubjDrive(e.target.value)} placeholder="https://..." className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-400" aria-label="Google Drive link (optional)"/>
                                    </div>
                                    <div>
                                        ...
                                        <input type="url" id="subj_github" value={subjGithub} onChange={(e) => setSubjGithub(e.target.value)} placeholder="https://..." className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-400" aria-label="GitHub link (optional)"/>
                                    </div>
                                </div>
                                <div className="flex justify-end pt-2"><button type="submit" className="px-6 py-2 bg-primary-600 text-white font-semibold rounded-lg shadow-md hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors" aria-label="Create subject">Create Subject</button></div>
                            </form>
                    ...
                    {activeTab === 'departments' && (
                         <section aria-labelledby="create-department-heading">
                            <form onSubmit={handleCreateDepartmentSubmit} className="space-y-4">
                                <h2 id="create-department-heading" className="text-2xl font-bold text-gray-800 dark:text-gray-200">Create New Department</h2>
                                <div>
                                    <label htmlFor="dept_name" className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Department Name</label>
                                    <input type="text" id="dept_name" value={deptName} onChange={(e) => setDeptName(e.target.value)} placeholder="e.g., Mechanical Engineering" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:placeholder-gray-400" aria-label="Department name"/>
                                </div>
                                <div className="flex justify-end pt-2"><button type="submit" className="px-6 py-2 bg-primary-600 text-white font-semibold rounded-lg shadow-md hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors" aria-label="Create department">Create Department</button></div>
                            </form>
