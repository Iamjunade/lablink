...
const ExperimentView: React.FC<ExperimentViewProps> = ({ experiment, onBack, onAddContribution, isAdmin, onDeleteContribution }) => {
  ...
  const filteredContributions = experiment.contributions.filter(c => c.type === activeTab);
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5 transition-shadow hover:shadow-md">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">{contribution.createdAt.toLocaleDateString()}</p>
                </div>
                <div className="flex items-center space-x-2">
                    <button className="flex items-center space-x-2 text-sm text-gray-500 hover:text-primary-600 font-medium p-2 -m-2 rounded-md transition-colors">
            ...
            <div className="mt-4">
                {contribution.type === ContributionType.Code && (
                    <pre className="bg-gray-900 text-white p-4 rounded-md text-sm overflow-x-auto"><code>{contribution.content}</code></pre>
                )}
                ...
                {contribution.type === ContributionType.Theory && (
                     <p className="text-gray-700 leading-relaxed">{contribution.content}</p>
                )}
